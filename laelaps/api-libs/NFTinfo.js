...
export async function getNFTinfo(
  ...
  let txns = res.transfers.filter(
    (txn) =>
      txn.rawContract.address ==
      nftAddress
  );

  if (txns.length == 0)
    return { message: "Not a holder" };
  const blockNumHex = txns[0].blockNum;
  const blockNumDecimal = fromHex(
    blockNumHex
  );
  ...
  const addressInDB =
    await getDocuments(userAddress);

  if (
    chatId &&
    addressInDB.length == 0
  ) {
    ...
    if (
      addressInDB[0].status !=
        statusObj.status ||
      addressInDB[0].validThru !=
        statusObj.validThru
    ) {
...
function isLessThanDaysAgo(
  ...
  return (
    ...
    daysAgo * secondsInADay
  );
}

function areTimestampsWithinDaysApart(
  timestamp1,
  timestamp2,
  daysApart
) {
...
function unixTimestampInFuture(
  baseTimestamp,
  daysInFuture
) {
  ...
  const millisecondsToAdd =
    daysInFuture * millisecondsInDay;
  const futureTimestampMilliseconds =
    baseTimestampMilliseconds +
    millisecondsToAdd;
  const futureTimestampSeconds =
    Math.floor(
      futureTimestampMilliseconds / 1000
    );
...
async function getDocuments(address) {
  try {
    const documents =
      await Subscription.findByAddress(
        address
      ).exec();
    return documents;
  } catch (error) {

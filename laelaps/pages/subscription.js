    ...
    (async () => {
      if (!isMismatched && userAddress) {
        const response = await fetch("/api/validate", {
          ...
          body: JSON.stringify({ userAddress, nftAddress }),
        });
        ...
        if (balances["laelaps"] > 5000000 || balances["masterKey"] > 0) {
          ...
          setMessage(
            "Wallet does not have enough $Laelaps or hold a Master Key."
          );
        }
  ...
  return (
    <div className={styles.square}>
      ...
      <div className={styles.valuesContainer}>
        ...
        <div className={styles.box}>Skull Key Balance: {balances["skull"]}</div>
      </div>
